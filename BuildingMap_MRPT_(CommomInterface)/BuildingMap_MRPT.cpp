...
RTC::ReturnCode_t BuildingMap_MRPT::onExecute(RTC::UniqueId ec_id)
{
	...
	if(m_InOdometryPositionIn.isNew()){
		...
		now_x = m_InOdometryPosition.data.position.x;		//m_InOdometryPosition.data.position.y;
		now_y = m_InOdometryPosition.data.position.y;		//m_InOdometryPosition.data.heading;
		now_theta = m_InOdometryPosition.data.heading;		//mapBuilder.getCurrentPoseEstimation()->getMeanVal().yaw();
	...
	if(m_rangeDataIn.isNew()){
		...
		std::cout << "100:" << m_rangeData.ranges[100] << std::endl;
	}
